...
import { cambiarPasswordSeguro } from '../actions'; // Asegúrate de tener esta función en actions.js
...
export default function AdminClientButtons({ registros }: AdminClientButtonsProps) {
  ...
  // --- FUNCIÓN EXPORTAR EXCEL CON ESTILOS ---
  const exportarExcel = () => {
    const encabezados = [
      "FECHA REGISTRO", "DOCUMENTO", "TIPO", "NOMBRE COMPLETO", 
      "GÉNERO", "CORREO", "TELÉFONO", "MUNICIPIO", "EPS", "ENLACES PDF"
    ];
    ...
    // Estilo Azul con Blanco para encabezados
    ...
    for (let R = range.s.r; R <= range.e.r; ++R) {
      for (let C = range.s.c; C <= range.e.c; ++C) {
        ...
        if (!ws[addr]) continue;
        ws[addr].s = (R === 0) ? estiloEncabezado : estiloCelda;
      }
    }

    ws['!cols'] = [{ wch: 15 }, { wch: 15 }, { wch: 10 }, { wch: 35 }, { wch: 10 }, { wch: 25 }, { wch: 15 }, { wch: 20 }, { wch: 15 }, { wch: 40 }];
    XLSX.utils.book_append_sheet(wb, ws, "Reporte");
    XLSX.writeFile(wb, "Reporte_Admin.xlsx");
  };

  // --- LÓGICA DE CAMBIO DE CLAVE CON BCRYPT ---
  const handleCambioPass = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      alert("Contraseña actualizada correctamente con Bcrypt.");
      setShowModal(false);
      setForm({ oldPass: '', newPass: '', confirmPass: '' });
      setMensaje({ text: '', isError: false });
    } else {
      setMensaje({ text: res.error, isError: true });
    }
  };

  return (
    <div className="flex items-center gap-3">
      <button onClick={() => setShowModal(true)} className="text-slate-400 hover:text-white text-xs border border-slate-700 px-3 py-2 rounded-lg transition-all flex items-center gap-2">
        <span>Seguridad</span>
      </button>

      <button onClick={exportarExcel} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-xs font-bold transition-all shadow-lg flex items-center gap-2">
        Excel
      </button>

      <a href="/logout" className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-xs font-bold">
        Salir
      </a>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl">
            <h3 className="text-2xl font-black text-gray-900 mb-6">Seguridad</h3>
            <form onSubmit={handleCambioPass} className="space-y-4">
              ...
              <div className="flex gap-2 pt-4">
                <button type="submit" disabled={loading} className="flex-1 bg-blue-600 text-white py-4 rounded-2xl font-bold hover:bg-blue-700 disabled:bg-gray-400">
